...
class Content
{
    ...
    public function appendGallery($source = "", $images = [], $modalName = "myModal"){
        ...
        foreach($images as $img => $index){
            $imgGrid .= "<div class='column'><img src='$source/$index' onclick='openModal();currentSlide($img+1)' class='hover-shadow'></div>";
        }
        $imgGrid .= "</div>";
        
        $imgModal = "<div id='$modalName' class='modal'><span class='close cursor' onclick='closeModal()'>&times;</span><div class='modal-content'>";
        foreach($images as $img => $index){
            $imgModal .= "<div class='mySlides'><div class='numberText'>$img+1 / ". count($images) 
        ...
        foreach($images as $img => $index){
            $imgModal .= "<div class='column'><img class='demo' src='$source/$index' onclick='currentSlide($img+1)' alt='$index'></div>";
        }
        $imgModal .= "</div></div>";
        $imgGrid .= $imgModal;
